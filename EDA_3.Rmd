...
The only thing that need's to control for in this case is time or seasonality as anything that changes over time is a potential source of endogeneity for a time related event.  
...
The *PCIC* or "Percentage of degrees awarded in a {specific field}" variables are  high column count but low in explanatory value, at least for our research question. Intuitively I cant reason how or why the percentage of degrees awarded for a particular field would be at all related to Google search traffic for a University, nor would it impact when the score card was released.
...
Just for good measure im going to run a very simply linear model to see if there are any variables worth looking into more. 
...
I'm choosing to keep the college "race/ gender flagged" variables though as this is likely to be a more constant race indicator less subject to change over time. 
...
My Null removal criteria is rather arbitrary, however im selecting a cut off at 25% Null, meaning that I'm rejecting any variables that have *more than 25% of schools* failing to report on them. 
...
We can see that this data is skewed to the right, indicating that while most schools "median earning 10 years after graduation" tends to fall right around $40,000 we have some outlire schools who report very high earnings above 150K.
...
$Note:$ Uur sample is lopsided (~80% of the records are before the score card)
...
- net avg priving ranges from 0 to 45774.00 wiht a mean of 14079.67
...
This does however suggest that I should be including the prive related variables in my models so that I dont add omitted variable bias to my earnings variable.  
...
$Note:$ This statisticaly significant coefficient would indicate that a for each additional **$1000** spend on school your *log(median earnings)* would increase by a **0.018**  which is approximately **1.8%** increase in expected earnings. The price of school is in no way correlated to the Score Card being released, however we should add the *NPT4_PRIV* variable to our model as it is obviously correlated to *earnings* which is central to our study. 
...
***
Additionally I was suprised that the *log_mean_10_yr_earnings* wasnt significant.
...
_The effect of releasing the *college score card* on *Google search volumes* is **0.080** points **higher for low earning schools** than it is for **higher earning colleges*.
...
$OverAllEffect:$ When the college score card released it increase search traffic for both the low and high earning colleges, however it increased the search traffic for low earning schools more so that it did for high earning schools.

$Note$: Findings are statistical significant